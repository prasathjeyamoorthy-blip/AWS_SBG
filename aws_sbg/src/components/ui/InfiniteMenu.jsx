...
function makeVertexArray(gl,bufLocNumElmPairs,indices){
  ...
  for(const[buffer,loc,numElem]of bufLocNumElmPairs){
    if(loc===-1)continue;gl.bindBuffer(gl.ARRAY_BUFFER,buffer);gl.enableVertexAttribArray(loc);gl.vertexAttribPointer(loc,numElem,gl.FLOAT,false,0,0);
  }
  if(indices){const indexBuffer=gl.createBuffer();gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER,indexBuffer);gl.bufferData(gl.ELEMENT_ARRAY_BUFFER,new Uint16Array(indices),gl.STATIC_DRAW);}
  gl.bindVertexArray(null);return va;
}
function resizeCanvasToDisplaySize(canvas){
  const dpr=Math.min(2,window.devicePixelRatio);
  const displayWidth=Math.round(canvas.clientWidth*dpr),displayHeight=Math.round(canvas.clientHeight*dpr);
  const needResize=canvas.width!==displayWidth||canvas.height!==displayHeight;
  if(needResize){canvas.width=displayWidth;canvas.height=displayHeight;}return needResize;
}
...
// ─── ArcballControl: exact original + one auto-spin injection ─────────────────
// The ONLY addition to the original is:
//   1. _autoSpinPaused flag toggled by pointer events
//   2. A single quat rotation applied at the TOP of update() when not paused
//   3. smoothRotationVelocity is overridden in #render to a fixed value during spin
class ArcballControl {
  ...
  _autoSpinPaused=false;_resumeTimer=null;
  ...
  constructor(canvas,updateCallback){
    ...
    canvas.addEventListener('pointerdown',e=>{
      ...
      this._autoSpinPaused=true;
      clearTimeout(this._resumeTimer);
    });
    canvas.addEventListener('pointerup',()=>{
      this.isPointerDown=false;
      clearTimeout(this._resumeTimer);
      this._resumeTimer=setTimeout(()=>{this._autoSpinPaused=false;},2000);
    });
  ...
  update(deltaTime,targetFrameDuration=16){
    ...
    // ── AUTO-SPIN: inject Y-axis rotation every frame when not paused ──────────
    if(!this._autoSpinPaused){
      const angle=0.008*timeScale; // ~0.46°/frame → smooth continuous rotation
      const spinQ=quat.setAxisAngle(quat.create(),vec3.fromValues(0,1,0),angle);
      this.orientation=quat.multiply(quat.create(),spinQ,this.orientation);
      quat.normalize(this.orientation,this.orientation);
      vec3.set(this.rotationAxis,0,1,0); // keep axis on Y for correct stretch direction
    }

    // ── ORIGINAL pointer logic (unchanged) ────────────────────────────────────
    if(this.isPointerDown){
      ...
      if(vec2.sqrLen(midPointerPos)>this.EPSILON){
        ...
        const p=this.#project(midPointerPos),q=this.#project(this.previousPointerPos);
        const a=vec3.normalize(vec3.create(),p),b=vec3.normalize(vec3.create(),q);
        vec2.copy(this.previousPointerPos,midPointerPos);
        angleFactor*=ANGLE_AMPLIFICATION;this.quatFromVectors(a,b,this.pointerRotation,angleFactor);
      }else{quat.slerp(this.pointerRotation,this.pointerRotation,this.IDENTITY_QUAT,INTENSITY);}
    }else{
      const INTENSITY=0.1*timeScale;
      quat.slerp(this.pointerRotation,this.pointerRotation,this.IDENTITY_QUAT,INTENSITY);
      // Only snap when user has paused auto-spin (just released pointer)
      if(this._autoSpinPaused&&this.snapTargetDirection){
    ...
    const s=Math.sin(rad/2.0);let rv=0;
    if(s>0.000001){
      rv=rad/(2*Math.PI);
  ...
  quatFromVectors(a,b,out,angleFactor=1){
    ...
    const angle=Math.acos(d)*angleFactor;quat.setAxisAngle(out,axis,angle);
...
// ─── InfiniteGridMenu: exact original, one change in #render ─────────────────
class InfiniteGridMenu {
  ...
  #initDiscInstances(count){
    ...
    for(let j=0;j<4;++j){
      ...
      gl.enableVertexAttribArray(loc);gl.vertexAttribPointer(loc,4,gl.FLOAT,false,bytesPerMatrix,j*4*4);
  ...
  #animate(deltaTime){
    ...
    positions.forEach((p,ndx)=>{
      ...
      const finalScale=s*scale;const matrix=mat4.create();
  ...
  #render(){
    ...
    // KEY FIX: during auto-spin, pass a constant w=0.008 so the shader always
    // computes rotationVelocity = min(.15, 0.008*15) = 0.12 → visible stretch.
    // During manual drag, use the real smoothRotationVelocity.
    const stretchW = this.control._autoSpinPaused
  ...
  #onControlUpdate(deltaTime){
    ...
    let damping=5/timeScale,cameraTargetZ=3*this.scaleFactor;
    ...
    if(!this.control.isPointerDown){
      ...
      // Only snap to nearest vertex when user has manually paused auto-spin
      if(this.control._autoSpinPaused){
      ...
      // ── Auto-zoom: oscillate between zoomed-out (full sphere) and zoomed-in ──
      // Period ~8 seconds: 4s zoom in, 4s zoom out
      if(!this.control._autoSpinPaused){
        const period = 8000; // ms for one full in-out cycle
        const t = (this.#time % period) / period; // 0..1
        // Smooth sine wave: 0 = zoomed out, 1 = zoomed in
        const zoom = (1 - Math.cos(t * 2 * Math.PI)) * 0.5;
        const zoomOut = this.scaleFactor * 5.5; // far: see all discs small
        const zoomIn  = this.scaleFactor * 2.2; // close: front disc fills view
        cameraTargetZ = zoomOut + (zoomIn - zoomOut) * zoom;
        damping = 80 / timeScale; // very slow, smooth camera movement
      }
    }else{
...
export default function InfiniteMenu({items=[],scale=1.0}){
  ...
  const handleButtonClick=()=>{
    ...
    else{const el=document.querySelector(activeItem.link);if(el)el.scrollIntoView({behavior:'smooth'});}
  };
